...
with ventilation_classification as
(
select
...
(
  select
      ...
      , case
          ...
          when MechVent=1 then
            (DATE_PART('day',charttime_lag-charttime)*24*60 + DATE_PART('hour',charttime_lag-charttime)*60 + DATE_PART('minute',charttime_lag-charttime)) / 60
          else null
        end as ventduration
      , LAG(Extubated,1)
      OVER
...
select icustay_id
  ...
  , ((DATE_PART('day',max(charttime)-min(charttime))*24*60 + DATE_PART('hour',max(charttime)-min(charttime))*60 + DATE_PART('minute',max(charttime)-min(charttime))) / 60) AS duration_hours
from vd2
